---
title: 通过历程发送直邮消息
description: 在Adobe Journey Optimizer中自动发送直邮。 了解文件路由设置、配置和计划，以实现无缝交付。
feature: Direct Mail
role: User
level: Intermediate
doc-type: Feature Video
duration: 188
last-substantial-update: 2026-01-24T00:00:00Z
jira: KT-20176
hide: true
hidefromtoc: true
source-git-commit: dcc1eee548f7e71ee71260ba8a14e04640c3fe2a
workflow-type: tm+mt
source-wordcount: '93'
ht-degree: 20%

---


# 通过历程发送直邮消息

了解如何利用Adobe Journey Optimizer中的直邮渠道在您的历程中自动安排直邮投放。 在历程画布上发现文件路由设置、配置创建频率并利用直邮操作。 此视频演示了AJO如何对参与者进行排队，如何定期生成带有时间戳的文件，以及在激活后自动发送这些文件。

>[!VIDEO](https://video.tv.adobe.com/v/3479162/?learn=on&enablevpops)

有关更多信息，请参阅[产品文档](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/channels/direct-mail/direct-mail-journeys)。
