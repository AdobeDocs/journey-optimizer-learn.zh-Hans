---
title: 为活动设置架构和数据集
description: 了解如何为Adobe Journey Optimizer营销活动定义架构、摄取数据和验证数据集。 在几分钟内获得有关个性化和执行的见解。
feature: Campaigns
role: Developer
level: Beginner
doc-type: Technical Video
duration: 502
last-substantial-update: '2026-05-02T00:00:00.000Z'
jira: KT-20979
exl-id: 4ff6cf29-181e-4e43-9ad7-922bc0d93304
autotag-review: '2026-05-18T17:42:19.393Z'
TQID: 'https://experienceleague.adobe.com/BwWdVQU4VVBCsFmP3xiPK6FoDZNSjJM-KOIzyLpskb8'
product_v2:
  - id: cb954087-f4fc-4456-afb9-e939cabcdc79
feature_v2:
  - id: b423a773-0a58-4a77-b65d-3dd4ae6ef841
role_v2:
  - id: ff6a42d2-313e-452e-93a6-792e4fad9ff8
level_v2:
  - id: e8ccd51f-da0d-4e3b-939b-e30d5ebb1ea5
topic_v2:
  - id: e0eb8757-182f-49f3-94a4-1587d16f5094
  - id: e1e0219c-f879-479f-8427-888ed2a6e9c2
source-git-commit: 880ae31cbaadba400f072d59c0b114978bb90fb5
workflow-type: tm+mt
source-wordcount: 105
ht-degree: 0%

---

# 为活动设置架构和数据集

了解如何在Adobe Journey Optimizer中为编排的活动设置关系架构和数据集。 本指南介绍如何定义键、关系和使用DDL文件高效地创建架构。 您还将了解如何通过CSV上载和变更数据捕获(CDC)来摄取数据、验证数据集，以及准备它们以进行查询、个性化和活动执行。 观看嵌入式视频，获取简化活动工作流程的分步说明。

>[!VIDEO](https://video.tv.adobe.com/v/3484901/?learn=on&enablevpops)
