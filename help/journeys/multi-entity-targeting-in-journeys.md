---
title: 在AJO中使用联合受众进行多实体定位
description: 了解如何在Adobe Journey Optimizer中使用具有联合受众的多实体定位功能，按订阅ID、帐户ID或订单ID来定位历程。
feature: Audiences
role: User
level: Beginner
doc-type: Feature Video
duration: 202
last-substantial-update: 2026-06-04
jira: KT-21475
autotag-review: '2026-06-05T00:20:38.082Z'
TQID: 'https://experienceleague.adobe.com/cdLjCbomgx7yqvjgIMpLnaMI93DXDUDMMSZk32rFmgY'
product_v2:
  - id: cb954087-f4fc-4456-afb9-e939cabcdc79
feature_v2:
  - id: d998adac-2f81-400b-a669-d07bb196e4eb
topic_v2:
  - id: bce87dde-a4ab-44c9-8a18-ad66e4ddb377
source-git-commit: 324d5fdbb23e15e2977ed8f4625248b9972ff583
workflow-type: tm+mt
source-wordcount: 120
ht-degree: 10%

---

# 在历程中对联合受众使用多实体定位

了解如何使用多实体定位超越基于用户档案的受众，并使用联合受众提供更相关的客户体验。

此视频演示如何按订阅ID、帐户ID或订单ID定位历程，以在单个客户与多个业务实体关联时实现更准确的受众激活和个性化参与。

有关此功能的详细信息，请访问[文档](https://experienceleague.adobe.com/zh-hans/docs/journey-optimizer/using/orchestrate-journeys/manage-journey/supplemental-identifier){target="_blank"}。

>[!VIDEO](https://video.tv.adobe.com/v/3491843/?captions=chi_hans&learn=on)

